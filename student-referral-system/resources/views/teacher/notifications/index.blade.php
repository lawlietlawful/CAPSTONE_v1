...
@section('page-sub', 'Updates on your referrals and reports')
...
<div class="bg-white rounded-2xl shadow-premium border border-gray-100 overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
        ...
        @if($unreadCount > 0)
            <form method="POST" action="{{ route('teacher.notifications.markAllRead') }}">
                @csrf
    ...
    <div class="divide-y divide-gray-100">
        @forelse($notifications as $notification)
            <a href="{{ route('teacher.notifications.show', $notification->id) }}"
               class="flex items-start gap-4 p-4 hover:bg-gray-50 transition-colors {{ !$notification->is_read ? 'bg-amber-50/40' : '' }}">
                ...
                @if(!$notification->is_read)
                    <span class="w-2 h-2 rounded-full bg-amber-500 flex-shrink-0 mt-2" title="Unread"></span>
                @endif
            </a>
        @empty
            <div class="p-12 text-center">
                ...
                <p class="text-gray-500 text-sm mt-1">You'll see updates here when a report escalates or a referral you filed changes status.</p>
            </div>
        @endforelse
    </div>
