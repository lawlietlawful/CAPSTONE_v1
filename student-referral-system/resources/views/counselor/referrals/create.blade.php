...
<div class="bg-white border border-gray-100 rounded-2xl shadow-sm max-w-4xl">
    ...
    <div class="p-6">
        <form action="{{ route('counselor.referrals.store') }}" method="POST" x-data="{ referralType: '{{ old('referral_type') }}' }">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="reason" id="reason" rows="4" required placeholder="Describe the concern or reason for referring this student..."
                        class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200 transition shadow-sm @error('reason') border-red-500 @enderror">{{ old('reason') }}</textarea>
            ...
            <div class="mt-8 flex items-center justify-end gap-3 pt-5 border-t border-gray-100">
                <a href="{{ route('counselor.referrals.index') }}" class="px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition">
                    Cancel
                </a>
